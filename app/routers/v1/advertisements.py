...
@router.get("/all_list", status_code=200, response_model=List[AdvertisementAPI])
...
@router.get("/get/{adv_id}", status_code=200, response_model=AdvertisementAPI)
async def get(adv_id: str, db = Depends(get_database)) -> AdvertisementAPI | JSONResponse:
    domain = AdvertisementDomain(db)
    return domain.get_advertisement(adv_id)


@router.post("/create", status_code=201, response_model=AdvertisementAPI)
async def create(adv_request: CreateAdvertisement, db = Depends(get_database)) -> AdvertisementAPI | JSONResponse:
    domain = AdvertisementDomain(db)
    return domain.create_advertisement(adv_request)


@router.patch("/update/{adv_id}", status_code=200, response_model=AdvertisementAPI)
async def update(adv_id: str, adv_request: UpdateAdvertisement, db = Depends(get_database)) -> AdvertisementAPI | JSONResponse:
    domain = AdvertisementDomain(db)
    return domain.update_advertisement(adv_id, adv_request)

    
@router.delete("/delete/{adv_id}", status_code=204)
